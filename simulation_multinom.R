...
predictor = seq(1,100)
test_dat_nonlinear=cbind(predictor,response)
test_dat_nonlinear=as.data.frame(test_dat_nonlinear)

# str(test_dat_nonlinear)

plt_dat = ggplot(data=test_dat_nonlinear,aes(x=predictor,y=response))
plt_dat = plt_dat+geom_point(color=response,alpha=1/2,size=5)
...
generate_multinom_linear= function(n){
  beta = rbind(c(1.38,-0.02),c(-1.44,0.005),c(0,0))
...
### scatter plot for linear data ###
test_dat = test_all_l[[2]]
response = rep(0,n)
for(i in 1:n){
  for(j in 1:3){
    if(test_dat[j,i]==1){
...
predictor = seq(1,100)
test_dat_linear=cbind(predictor,response)
test_dat_linear=as.data.frame(test_dat_linear)

plt_dat = ggplot(data=test_dat_linear,aes(x=predictor,y=response))
plt_dat = plt_dat+geom_point(color=response,alpha=1/2,size=5)
plt_dat = plt_dat+labs(title="Data Generated by Function 'generate'")
plt_dat = plt_dat+labs(x='Predictor Sequence')+labs(y='Outcome')
plt_dat
